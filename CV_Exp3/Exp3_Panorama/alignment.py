...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    for i in range(nRANSAC):
        new_match = []
        if m == eTranslate:
            a = random.sample(range(0,num_matches),1)
            x1,y1 = f1[matches[a[0]].queryIdx].pt
            x2,y2 = f2[matches[a[0]].trainIdx].pt
            tmp = np.array([[1, 0, x1-x2], [0, 1, y1-y2], [0, 0, 1]])
        elif m == eHomography:
            a = random.sample(range(0,num_matches),4)
            for j in a:
                new_match.append(matches[j])
            tmp = computeHomography(f1,f2,new_match)
        now_match = getInliers(f1,f2,matches,tmp,RANSACthresh)
        if  len(now_match) > max_match:
            if m == eTranslate:
                best_match.append(now_match)
            elif m == eHomography:
                best_match = now_match
            max_match = len(now_match)
        else :
            continue

    if m == eTranslate:
        M = leastSquaresFit(f1, f2, matches, eTranslate, best_match)
    elif m == eHomography:
        M = leastSquaresFit(f1, f2, matches, eHomography, best_match)
    # raise Exception("TODO in alignment.py not implemented")
    #TODO-BLOCK-END
    #END TODO
    return M
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        #TODO-BLOCK-BEGIN
        a = f1[matches[i].queryIdx].pt
        b = f2[matches[i].trainIdx].pt
        la = np.array([[*a,1]]).T # 转成齐次坐标
        lb = np.array([[*b,1]]).T
        la = np.dot(M,la)
        la /= la[2][0]  # 转回平面坐标
        lb /= lb[2][0]
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        for i in range(len(inlier_indices)):
            ...
            x2,y2 = f2[matches[i].trainIdx].pt     # 由于数据集中镜头是从左向右转，所以一定要2-1
            u += x2-x1
            v += y2-y1
            # raise Exception("TODO in alignment.py not implemented")
            #TODO-BLOCK-END
        ...
        new_match = []
        for i in range(len(inlier_indices)):
            new_match.append(matches[inlier_indices[i]])
...
if __name__ == '__main__':
    A = test.TestAlignment()
    A.setUp()
    A.test_computehomography2()
    A.test_computehomography3()
    A.test_alignPair()
    A.test_getInliers()
    A.test_leastSquaresFit()