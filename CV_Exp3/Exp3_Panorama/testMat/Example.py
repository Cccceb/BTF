...
def accumulateBlend(img, acc, M, blendWidth):
    ...
    coordinates = np.dstack((x_grid, y_grid, ones)) # 生成所有点的齐次坐标
    coordinates = coordinates.reshape((col * row, 3))
    coordinates = coordinates.T

    location = np.linalg.inv(M).dot(coordinates)    # M的逆来反向卷绕
...
def getAccSize(ipv):
    ...
    for i in ipv:
        ...
        mset = imageBoundingBox(img, M)
        # print(mset)
        minX = min(minX, mset[0])
        minY = min(minY, mset[1])
        maxX = max(maxX, mset[2])
        maxY = max(maxY, mset[3])
        #raise Exception("TODO in blend.py not implemented")
        #TODO-BLOCK-END
        # END TODO
