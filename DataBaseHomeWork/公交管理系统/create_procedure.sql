...
CREATE  PROCEDURE 	insert_siji(IN xm VARCHAR(20),IN xb CHAR(2),IN c VARCHAR(20),IN cd INTEGER,IN xl INTEGER)
BEGIN 
	INSERT INTO siji (`sj_name`,`sex`,`che_id`,`cd_id`,`xl_id`)
	SELECT xm,xb,c,cd,xl
	FROM DUAL
	WHERE EXISTS (
	SELECT * 
	FROM xianlu
	WHERE cd_id = cd AND xl_id = xl 
	) AND EXISTS(
	SELECT *
	FROM che
	WHERE che_id = c AND cd_id = cd
	);
END $
#插入车表数据	
DROP PROCEDURE insert_che $
CREATE PROCEDURE insert_che(IN che_id VARCHAR(20),IN zw INT,IN cd_id INT )
BEGIN
	INSERT INTO che (che_id,zw,cd_id) VALUES 
	(che_id,zw,cd_id);
END $
...
BEGIN 
	SELECT sj_name,`time`,zd,wz_name
	FROM siji,(
